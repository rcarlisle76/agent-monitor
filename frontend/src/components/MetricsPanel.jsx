...
function MetricCard({ label, value, color, warning }) {
  return (
    <div className={`rounded-lg border ${color} p-5 ${warning ? 'ring-1 ring-red-500' : ''}`}>
      ...
      <p className={`text-3xl font-bold ${warning ? 'text-red-400' : 'text-gray-100'}`}>{value}</p>
    </div>
  )
}
...
function AccuracyBar({ value }) {
  ...
  return (
    <div className="flex items-center gap-3">
      ...
      <span className="text-lg font-bold text-gray-100 font-mono w-16 text-right">
        {pct.toFixed(1)}%
      </span>
    </div>
  )
...
export default function MetricsPanel({ lastEvent }) {
  ...
  return (
    <div className="p-6 space-y-6 overflow-y-auto h-full">
      {m.flagged_agents > 0 && (
        <div className="rounded-lg border border-red-700 bg-red-950 px-5 py-3 flex items-center gap-3">
          <span className="text-red-400 text-xl">⚑</span>
          <div>
            <p className="text-red-300 font-semibold text-sm">
              {m.flagged_agents} agent{m.flagged_agents > 1 ? 's' : ''} flagged for termination
            </p>
            <p className="text-red-600 text-xs">Accuracy dropped below 70% threshold</p>
          </div>
        </div>
      )}
      ...
      <div className="rounded-lg border border-gray-800 bg-gray-900 p-5 space-y-4">
        ...
        <div className="border-t border-gray-800 pt-4">
          ...
          <p className="text-xs text-gray-600 mt-1">for completed agents</p>
        </div>

        <div className="border-t border-gray-800 pt-4">
          <p className="text-xs uppercase tracking-widest text-gray-400 mb-1">Total Tasks</p>
          <p className="text-2xl font-bold text-gray-100">{m.total_tasks}</p>
        </div>
      </div>
    </div>
  )
